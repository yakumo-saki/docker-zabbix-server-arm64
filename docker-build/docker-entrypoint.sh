...
check_db_connect_mysql() {
    ...
    WAIT_TIMEOUT=5
    PING_CMD="mysqladmin ping -h ${DB_SERVER_HOST} -P ${DB_SERVER_PORT} -u ${DB_SERVER_ZBX_USER}"
    PING_CMD="$PING_CMD --password=""${DB_SERVER_ZBX_PASS}"" --silent --connect_timeout=10)"

    while [ ! "$(mysqladmin ping -h ${DB_SERVER_HOST} -P ${DB_SERVER_PORT} -u ${DB_SERVER_ZBX_USER} \
                --password="${DB_SERVER_ZBX_PASS}" --silent --connect_timeout=10)" ]; do
